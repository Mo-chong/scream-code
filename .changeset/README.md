...
| --- | --- | --- |
| `scream-code` | `apps/scream-code` | CLI / TUI application — provides the `scream` command after install |
| `@scream-code/scream-code-sdk` | `packages/node-sdk` | Public TypeScript SDK |
...
- `@scream-code/scream-code-oauth`
- `@scream-code/jian`
- `@scream-code/ltod`
- `@scream-code/vis`
- `@scream-code/vis-server`
- `@scream-code/vis-web`

Version impact from internal dependencies must be judged manually. The published artifacts for CLI and SDK bundle internal workspace packages into the artifact itself; runtime `dependencies` of published packages must not include any `@scream-code/*` internal workspace packages.
...
| --- | --- |
| Only modifies TUI behavior in `scream-code` | Add `patch` / `minor` / `major` to `scream-code` |
| Only modifies internal packages, no user-visible change in SDK / CLI | Usually no changeset needed |
| Internal package fix changes the CLI user experience | Add a changeset to `scream-code` describing the user-visible fix |
| Internal package adds a new capability exposed by the SDK | Add a changeset to `@scream-code/scream-code-sdk` |
| SDK behavior change affects CLI user experience | Add changesets to both `@scream-code/scream-code-sdk` and `scream-code` |
| Provider abstraction change affects SDK / CLI | Add changesets to the affected `@scream-code/scream-code-sdk` and/or `scream-code` |
...
1. Open each publishable package's page on the npm website, e.g. `https://www.npmjs.com/package/scream-code`.
...
- Do not add release changesets for private internal packages; only select `scream-code` and `@scream-code/scream-code-sdk`.
- If a change in an underlying internal package alters user-visible behavior or public API of a publishable package, add a changeset to the affected publishable package. For example, when a bug fixed in `@scream-code/agent-core` resolves an issue CLI users encounter, add a changeset to `scream-code` describing the user-visible fix.
- `scream-code` is the official CLI package name; after a global install it provides the `scream` command.
